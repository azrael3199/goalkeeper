...
class _SplashTransitionState extends State<SplashTransition> {
  ...
  void initState() {
    super.initState();
    Timer(const Duration(seconds: 3), () => Navigator.pushReplacement(context, MaterialPageRoute(builder:
                                                          (context) => 
                                                          const SecondScreen()
                                                         ))
                                                        );
  ...
  Widget build(BuildContext context) {
    return Container(
      color: Colors.white,
      child: Image.asset('assets/images/logo_main.png', height: MediaQuery.of(context).size.height),
    );
  }
